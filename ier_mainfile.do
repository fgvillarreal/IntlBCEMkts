...
* merge with NP interest rates
qui merge 1:1 ISO date using $main_path\ier2018_np_data.dta, generate(_merge_np)
...
qui replace r_np = r_np/100 if ISO ~= "ARG"
qui replace ydefl_np = log(ydefl_np)
qui gen inflation_np = d.ydefl_np
qui gen irate_np = r_np - (0.25*inflation_np + 0.25*l.inflation_np + 0.25*l2.inflation_np + 0.25*l3.inflation_np)
...
keep ISO IFScode date debt_gdp NGama-s_1 rex rex_used rer_usa ynom xrate_regime Country irate_np `AGvars'
